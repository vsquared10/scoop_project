...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    userCommentIds.splice(userCommentIds.indexOf(id), 1);
    const artCommentIds = database.articles[savedComment.articleId].commentIds;
    artCommentIds.splice(artCommentIds.indexOf(id), 1);
    response.status = 204;
  } else {
    response.status = 404;
  };
...
function loadDatabase() {
  try {
    var doc = yaml.safeLoad(fs.readFileSync('scoop.yml', 'utf8'));
    return doc;
  } catch (e) {
    console.log(e);
  }
...
function saveDatabase() {
  var db = yaml.safeDump(database);
  fs.writeFile("scoop.yml", db, (err) => {
    if (err) {
        console.error(err);
        return;
    };
